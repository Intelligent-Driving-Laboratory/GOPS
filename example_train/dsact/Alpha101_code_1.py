...
def decay_linear(df, period=10):
    ...
    # na_series = df.values  # 使用.values属性获取numpy数组
...
class Alphas(object):
    def __init__(self, df_data):
        ...
        # 假设df是包含OHLCV信息的DataFrame，且包含'High', 'Low', 'Close'和'Volume'列
        df_data['Typical_Price'] = (df_data['high'] + df_data['low'] + df_data['close']) / 3
        df_data['VWAP'] = (df_data['Typical_Price'] * df_data['volume']).cumsum() / df_data['volume'].cumsum()

        self.returns = df_data['Returns'] 
        self.vwap = (df_data['VWAP']*1000)/(df_data['volume']*100+1) 
