...
class DSAC2(AlgorithmBase):
    ...
    def __compute_loss_q(self, data: DataDict):
        ...
        q1, q1_std, q1_sample = self.__q_evaluate(obs, act, self.networks.q1, use_min=False)
        q2, q2_std, q2_sample = self.__q_evaluate(obs, act, self.networks.q2, use_min=False)

        q1_next, _, q1_next_sample = self.__q_evaluate(
            obs2, act2, self.networks.q1_target, use_min=False
        )
        
        q2_next, _, q2_next_sample = self.__q_evaluate(
            obs2, act2, self.networks.q2_target, use_min=False
        )
        ...
        target_q1, target_q1_bound = self.__compute_target_q(
            rew, done, q1.detach(),q_next.detach(), q_next_sample.detach(), log_prob_act2.detach(),
        )
        
        target_q2, target_q2_bound = self.__compute_target_q(
            rew, done, q2.detach(),q_next.detach(), q_next_sample.detach(), log_prob_act2.detach(),
        )
        ...
        if self.bound:
            ...
            q1_loss = -Normal(q1, q1_std).log_prob(target_q1).mean()
            q2_loss = -Normal(q2, q2_std).log_prob(target_q2).mean()
    ...
    def __compute_loss_policy(self, data: DataDict):
        ...
        q1, _, _ = self.__q_evaluate(obs, new_act, self.networks.q1, use_min=False)
        q2, _, _ = self.__q_evaluate(obs, new_act, self.networks.q2, use_min=False)
