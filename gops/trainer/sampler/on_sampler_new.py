...
class OnSamplerNew():
    def __init__(self, **kwargs):
        ...
        self.mb_obs = np.zeros((self.sample_batch_size, ) + self.obs_dim, dtype=np.float32)
    ...
    def sample_with_replay_format(self):
        ...
        for t in range(self.sample_batch_size):
            ...
            self.mb_obs[t], self.mb_act[t], self.mb_rew[t], self.mb_done[t], self.mb_tlim[t], self.mb_logp[t], self.mb_val[t] = \
                self.obs.copy(), action, reward, self.done, info['TimeLimit.truncated'], logp, value
            self.obs = next_obs
            if self.done or info['TimeLimit.truncated']:
                self.obs = self.env.reset()
            if self.done or info['TimeLimit.truncated'] or t == self.sample_batch_size - 1:
                last_obs_expand = torch.from_numpy(np.expand_dims(next_obs, axis=0).astype('float32'))
                est_last_value = self.networks.value(last_obs_expand).detach().item() * (1 - self.done)
                ptr = t
                self._finish_trajs(est_last_value, last_ptr, ptr)
                last_ptr = t
        end_time = time.perf_counter()
        tb_info[tb_tags["sampler_time"]] = (end_time - start_time) * 1000
        mb_data = {'obs': torch.from_numpy(self.mb_obs),
