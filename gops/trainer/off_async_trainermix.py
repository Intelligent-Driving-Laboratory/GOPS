...
class OffAsyncTrainermix():
    ...
    def _set_algs(self):
        ...
        for alg in self.algs:
            ...
            self.learn_tasks.add(alg, alg.update_policy.remote(data, self.iteration))  # 用采样结果给learner添加计算梯度的任务
    ...
    def step(self):
        ...
        for alg, objID in self.learn_tasks.completed():
            ...
            self.learn_tasks.add(alg, alg.update_policy.remote(data, self.iteration))  # 将完成了的learner重新算梯度
            self.iteration += 1
            # log
                ...
                if self.iteration % 50 == 0:
                    ...
                    for j in range(i):
                        ...
                        if j == 0:
                            ...
                            values_last_time = [a + b for a, b in zip(values_last_time, values)]
                            values_last_time = [a / (j + 1) for a in values_last_time]  # error
                            weights_last_time = dict(zip(keys, values_last_time))
