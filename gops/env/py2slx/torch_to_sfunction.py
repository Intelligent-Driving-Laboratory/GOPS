...
def network_output(obs):
    ...
    args.update(
        dict(
            action_high_limit=np.array([30.0], dtype=np.float32),
            action_low_limit=np.array([-30.0], dtype=np.float32),
        )
    )
    ...
    state_dict.update(
        dict(act_high_lim=policy.act_high_lim, act_low_lim=policy.act_low_lim)
    )  # if some keys in state_dict are missing, add it!
