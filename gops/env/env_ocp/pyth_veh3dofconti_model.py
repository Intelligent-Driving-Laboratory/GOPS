...
class Veh3dofcontiModel(PythBaseModel):
    ...
    def forward(self, obs: torch.Tensor, action: torch.Tensor, done: torch.Tensor, info: InfoDict) \
        ...
        ego_obs = torch.concat((next_state[:, :4] - next_ref_points[:, 0], next_state[:, 4:]), dim=1)
        ref_obs = (next_state[:, :2].unsqueeze(1) - next_ref_points[:, 1:, :2]).reshape((-1, self.pre_horizon * 2))
