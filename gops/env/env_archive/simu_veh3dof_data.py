...
class SimuVeh3dof(gym.Env, ):
    def __init__(self, **kwargs):
        ...
        self.obs_scale = np.array(kwargs["obs_scale"])
        self.act_scale = np.array(kwargs["act_scale"])
    ...
    def postprocess(self, state):
        ...
        if self.use_ref =="Pos":
            x_pre = state[0] + ref_v * 0.01 * self.act_repeat * np.linspace(1, self.ref_horizon, self.ref_horizon)
