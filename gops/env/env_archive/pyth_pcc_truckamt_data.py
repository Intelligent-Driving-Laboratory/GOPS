...
class TruckParameter(Structure):
    """
    Truck Position Structure for C/C++ interface
    """
    _fields_ = [
...
class PythPCCTruckAMTModel(gym.Env):
    ...
    def __init__(self, road_map=RoadMap(),Truck_parameter=TruckParameter(),**kwargs):
        ...
        Truck_parameter.M_SU = 4455 # 悬上质量，kg
        Truck_parameter.IZZ_SU = 34802.6 # 转动惯量，kg*m^2
        Truck_parameter.A = 6.8 # 迎风面积，m^2
        Truck_parameter.CFx = 0.3981 * 2 / 1.225 # 空气动力学侧偏角为零度时的纵向空气阻力系数
        Truck_parameter.AV_ENGINE_IDLE = 725 # 怠速转速，rpm
        Truck_parameter.IENG = 1.4 # 曲轴转动惯量，kg*m^2
        Truck_parameter.TAU = 0.1 # 发动机-变速箱输入轴 时间常数，s
        Truck_parameter.R_GEAR_TR1 = 7.59 # 最低档变速箱传动比
        Truck_parameter.R_GEAR_FD = 5.0 # 主减速器传动比
        Truck_parameter.BRAK_COEF = 1 # 液压缸变矩系数,Nm/(MPa)
        Truck_parameter.Steer_FACTOR = 25 # 转向传动比
        Truck_parameter.M_US = 1265 # 簧下质量，kg
        Truck_parameter.RRE = 0.51 # 车轮有效滚动半径，m
        Truck_parameter.CF = -80000 # 前轮侧偏刚度，N/rad
        Truck_parameter.CR = -140000 # 后轮侧偏刚度，N/rad
        Truck_parameter.ROLL_RESISTANCE = 0.015 # 滚动阻力系数
        ...
        self.dll.init(c_float(x), c_float(y),
                      ...
                      c_float(self.step_length), byref(Truck_parameter))
        self.v_target = 30
        self.Np = 50
    ...
    def step(self, action=None):
        ades = action
        steer_front = 0 #steer front ,default = 0
        if ades is None:
...
def env_creator(**kwargs):
    return TimeLimit(PythPCCTruckAMTModel(**kwargs), 20000)

if __name__ == "__main__":
    env = PythPCCTruckAMTModel()
    for i in range(10):
