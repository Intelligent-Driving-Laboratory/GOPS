...
class RoadMap:
    def __init__(self):
        self.path = os.path.join(
            ...
            "resources/pyth_pcc_trucklcf_file/Roadmap.csv",
        )
...
class PythPccTrucklcfModel(torch.nn.Module):
    ...
    def initialize_state(self):
        self.d_init_state = torch.empty(
            [self.sample_batch_size, self.dynamic_state_dim]
        )
        self.d_init_state[:, 0] = (
            torch.normal(
                0.0,
        ...
        self.d_init_state[:, 1] = torch.linspace(
            self.min_action, self.max_action, self.sample_batch_size
        )
        self.d_init_state[:, 2] = torch.linspace(0, 10000, self.sample_batch_size)
        return self.d_init_state

    def forward(
        self, state: torch.Tensor, action: torch.Tensor, beyond_done=torch.tensor(0)
    ):
        """
        ...
        theta = torch.from_numpy(
            np.interp(d_x.detach().numpy(), self.map_x, self.map_Theta)
        )
        # 换挡等
        ...
        d_v_next = d_v + self.dynamic_T * (
            d_Te * self.ig * self.i0 * self.etaT / (self.r * self.m)
            - 0.5 * self.Cd * self.A * self.rho * (d_v ** 2) / self.m
            - self.gravity * (self.f * torch.cos(theta) + torch.sin(theta))
        )
        d_Te_next = d_Te + (action[:, 0] - d_Te) / self.tau_eng * self.dynamic_T
        d_x_next = d_x + self.dynamic_T * d_v
        dynamic_state_next = torch.stack([d_v_next, d_Te_next, d_x_next]).transpose(
            1, 0
        )
        ...
        future_ds_list = [
            d_x_next + d_v_next * self.dynamic_T * i for i in range(self.Np)
        ]
        future_thetas_list = np.array(
            [
                np.interp(
                    future_ds_list[i].detach().numpy(), self.map_x, self.map_Theta
                )
        ...
        # reward
        reward = -(
            weight_fuel * self.Qfuel
            + weight_tracking * ((d_v - self.v_target) ** 2)
            + weight_control * ((action - self.ades_k) ** 2)
        )
    ...
    def forward_n_step(
        self, state: torch.Tensor, action, step, beyond_done=torch.tensor(0)
    ):
    ...
    def check_done(self, state):
        ...
        threshold = np.kron(
            np.ones([self.sample_batch_size, 1]), np.array([self.v_threshold])
        )  # 矩阵乘法 A_n×m*B_p×q=Cnp*mq ,self.x_range
        threshold = np.array(threshold, dtype="float32")
        check_state = state[:, 0].clone() - self.v_target
        check_state.detach_()
        sign_error = torch.sign(
            torch.abs(check_state) - threshold
        )  # if abs state is over threshold, sign_error = 1
        self._reset_index, _ = torch.max(
            sign_error, 1
        )  # if one state is over threshold, _reset_index = 1
        for i in range(self.sample_batch_size):
        ...
        for i in range(len(EngT)):
            ...
            if index_torq == 0:
                if torq_temp[1] - torq_temp[0] == 0.0:
                    ...
                    throttle = 0.0 + (Te - torq_temp[0]) / (
                        torq_temp[1] - torq_temp[0]
                    ) * (0.1 - 0)
            elif index_torq == 1:
        ...
        for i in range(len(gear_input)):
            ...
            if geari == 1:
                if AT_Speed[i].item() < down_th[i].item():
                    ...
                    down_th[i].item() >= AT_Speed[i].item()
                    and AT_Speed[i].item() < up_th[i].item()
                ):
                    self.gear = 1
    ...
    def Threshold_LU(self, gear_input: torch.Tensor, throttle: torch.Tensor):
        ...
        for i in range(len(gear_input)):
            ...
            if geari == 1:  # 1-2
                up = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [158, 158, 675, 675]
                )
                down = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [70, 70, 110, 110]
                )
            elif geari == 2:
                up = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [237, 237, 1255, 1255]
                )
                down = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [213, 213, 450, 450]
                )
            elif geari == 3:
                up = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [356, 356, 1860, 1860]
                )
                down = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [320, 320, 710, 710]
                )
            elif geari == 4:
                up = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [633, 633, 2490, 2490]
                )
                down = np.interp(
                    throttle[i].item(), [0, 0.75, 0.9, 1], [480, 480, 1067, 1067]
                )
            elif geari == 5:
                up = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [800, 800, 3080, 3080]
                )
                down = np.interp(
                    throttle[i].item(), [0, 0.75, 0.9, 1], [620, 620, 1250, 1250]
                )
            else:
                up = np.interp(
                    throttle[i].item(), [0, 0.2, 0.8, 1], [1067, 1067, 3080, 3080]
                )
                down = np.interp(
                    throttle[i].item(), [0, 0.75, 0.9, 1], [960, 960, 1250, 1250]
                )
            up_th[i] = up
            down_th[i] = down
