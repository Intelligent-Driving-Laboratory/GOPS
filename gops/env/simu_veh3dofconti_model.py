...
class SimuVehicle3dofcontiModel(torch.nn.Module):
    def __init__(self):
        ...
        self.state_dim = 6
        self.action_dim = 2
        lb_state = [self.min_x, self.min_y, self.min_u, self.min_v, self.min_phi, self.min_omega]
        hb_state = [self.max_x, self.max_y, self.max_u, self.max_v, self.max_phi, self.max_omega]
        lb_action = [self.min_steel, self.min_force, self.min_force, self.min_force, self.min_force]
        hb_action = [self.max_steel, self.max_force, self.max_force, self.max_force, self.max_force]
    ...
    def forward(self, state: torch.Tensor, action: torch.Tensor, beyond_done=torch.tensor(1)):
        ...
        state_next[:, 0] = state[:, 0] + self.dt * (state[:, 2] * torch.cos(state[:, 4]) - state[:, 3] * torch.sin(state[:, 4]))  # "X"
        state_next[:, 1] = state[:, 1] + self.dt * (state[:, 3] * torch.cos(state[:, 4]) + state[:, 2] * torch.sin(state[:, 4]))  # "Y"
        state_next[:, 2] = state[:, 2] + self.dt * act_acc  # "u"
        state_next[:, 3] = (self.mass * state[:, 2] * state[:, 3] + self.dt * (self.lf * self.kf - self.lr * self.kr) *
                        state[:, 5] - self.dt * self.kf * act_steer * state[:, 2] - self.dt * self.mass * (state[:, 2] ** 2) *
                        state[:, 5]) / (self.mass * state[:, 2] - self.dt * (self.kf + self.kr))  # "v"
        state_next[:, 4] = state[:, 4] + self.dt * state[:, 5]  # "Phi"
        state_next[:, 5] = (self.Izz * state[:, 2] * state[:, 5] + self.dt * (self.lf * self.kf - self.lr * self.kr) *
                        state[:, 3] - self.dt * self.lf * self.kf * act_steer * state[:, 2]) / (self.Izz *
                        state[:, 2] - self.dt * (self.lf * self.lf * self.kf + self.lr * self.lr * self.kr))  # "r"
        # state_next = state_next.transpose(0, 1)

        ################################################################################################################
        # define the ending condation here the format is just like isdone = l(next_state)
        isdone = (state_next[:, 0] < self.min_x) + \
        ...
        reward = -pow((state_next[:, 2] - 5), 2) - pow((state_next[:, 1] - 3 * torch.sin(state_next[:, 0])), 2) - \
...
if __name__ == '__main__':
    ...
    state = torch.tensor([[0., 0., 0.0001, 0., 0., 0.], [0., 0., 0.0001, 0., 0., 0.]])
    ...
    for step in range(1000):
        ...
        action = torch.tensor([[steer, 0., acc, 0., 0.], [steer, 0.,acc, 0., 0.]])
