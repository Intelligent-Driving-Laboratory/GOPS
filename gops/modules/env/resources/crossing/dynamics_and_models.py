...
from modules.env.resources.crossing.endtoend_env_utils import rotate_coordination, L, W, L_BIKE, W_BIKE, CROSSROAD_SIZE, LANE_WIDTH, LANE_NUMBER, \
...
class EnvironmentModel(object):  # all tensors
    ...
    def compute_rewards(self, obses_ego, obses_bike, obses_person, obses_veh, actions):
        ...
        devi_phi = -torch.square(tracking_infos[:, 1] * np.pi / 180.)
        devi_v = -torch.square(tracking_infos[:, 2])

        # rewards related to veh2veh collision
        ego_lws = (L - W) / 2.
        ego_front_points = ego_infos[:, 3] + ego_lws * torch.cos(ego_infos[:, 5] * np.pi / 180.), \
                           ego_infos[:, 4] + ego_lws * torch.sin(ego_infos[:, 5] * np.pi / 180.)
        ego_rear_points = ego_infos[:, 3] - ego_lws * torch.cos(ego_infos[:, 5] * np.pi / 180.), \
                          ego_infos[:, 4] - ego_lws * torch.sin(ego_infos[:, 5] * np.pi / 180.)
        veh2veh4real = torch.zeros_like(veh_infos[:, 0])
        ...
        for veh_index in range(int(veh_infos.shape[1] / self.per_veh_info_dim)):
            vehs = veh_infos[:, veh_index * self.per_veh_info_dim:(veh_index + 1) * self.per_veh_info_dim]
            veh_lws = (L - W) / 2.
            veh_front_points = vehs[:, 0] + veh_lws * torch.cos(vehs[:, 3] * np.pi / 180.), \
                              vehs[:, 1] + veh_lws * torch.sin(vehs[:, 3] * np.pi / 180.)
            veh_rear_points = vehs[:, 0] - veh_lws * torch.cos(vehs[:, 3] * np.pi / 180.), \
                              vehs[:, 1] - veh_lws * torch.sin(vehs[:, 3] * np.pi / 180.)
            for ego_point in [ego_front_points, ego_rear_points]:
                for veh_point in [veh_front_points, veh_rear_points]:
                    veh2veh_dist = torch.sqrt(torch.square(ego_point[0] - veh_point[0]) + torch.square(ego_point[1] - veh_point[1]))
        ...
        for bike_index in range(int(bike_infos.shape[1] / self.per_bike_info_dim)):
            ...
            bike_front_points = bikes[:, 0] + bike_lws * torch.cos(bikes[:, 3] * np.pi / 180.), \
                                bikes[:, 1] + bike_lws * torch.sin(bikes[:, 3] * np.pi / 180.)
            bike_rear_points = bikes[:, 0] - bike_lws * torch.cos(bikes[:, 3] * np.pi / 180.), \
                               bikes[:, 1] - bike_lws * torch.sin(bikes[:, 3] * np.pi / 180.)
            for ego_point in [ego_front_points, ego_rear_points]:
    ...
    def compute_next_obses(self, obses_ego, obses_bike, obses_person, obses_veh, actions):
        ...
        if self.mode != 'training':
            ...
            next_tracking_infos = torch.zeros(len(next_ego_infos), (self.num_future_data+1)*self.per_tracking_info_dim)
            ...
            for ref_idx, path in enumerate(self.ref_path.path_list):
                ...
                tracking_info_4_this_ref_idx = self.ref_path.tracking_error_vector(next_ego_infos[:, 3],
                                                                                           ...
                                                                                           next_ego_infos[:, 0],
                                                                                           self.num_future_data)
                next_tracking_infos = torch.where(ref_indexes == ref_idx, tracking_info_4_this_ref_idx,
                                               next_tracking_infos)
    ...
    def person_predict(self, person_infos):
        ...
        for persons_index in range(len(person_mode_list)):
            ...
            next_person_phis_rad = torch.where(next_person_phis_rad > np.pi, next_person_phis_rad - 2 * np.pi, next_person_phis_rad)
            next_person_phis_rad = torch.where(next_person_phis_rad <= -np.pi, next_person_phis_rad + 2 * np.pi, next_person_phis_rad)
            next_person_phis = next_person_phis_rad * 180 / np.pi
            next_person_index = person_index
            pred.append(torch.stack([next_person_xs, next_person_ys, next_person_vs, next_person_phis, next_person_index], 1))
    ...
    def predict_for_bike_mode(self, bikes, mode):
        ...
        if mode in ['dl_b', 'rd_b', 'ur_b', 'lu_b']:
            bike_phis_rad_delta = torch.where(middle_cond, (bike_vs / (CROSSROAD_SIZE/2+3*LANE_WIDTH + BIKE_LANE_WIDTH / 2)) / self.base_frequency, zeros)
        elif mode in ['dr_b', 'ru_b', 'ul_b', 'ld_b']:
            bike_phis_rad_delta = torch.where(middle_cond, -(bike_vs / (CROSSROAD_SIZE/2-3.0*LANE_WIDTH - BIKE_LANE_WIDTH / 2)) / self.base_frequency, zeros)  # TODO：ONLY FOR 3LANE
        else:
        ...
        next_bike_phis_rad = torch.where(next_bike_phis_rad > np.pi, next_bike_phis_rad - 2 * np.pi, next_bike_phis_rad)
        next_bike_phis_rad = torch.where(next_bike_phis_rad <= -np.pi, next_bike_phis_rad + 2 * np.pi, next_bike_phis_rad)
        next_bike_phis = next_bike_phis_rad * 180 / np.pi
        next_bike_index = bike_index
        return torch.stack([next_bike_xs, next_bike_ys, next_bike_vs, next_bike_phis, next_bike_index], 1)
    ...
    def predict_for_veh_mode(self, vehs, mode):
        ...
        if mode in ['dl', 'rd', 'ur', 'lu']:
            veh_phis_rad_delta = torch.where(middle_cond, (veh_vs / (CROSSROAD_SIZE/2+0.5*LANE_WIDTH)) / self.base_frequency, zeros)
        elif mode in ['dr', 'ru', 'ul', 'ld']:
            veh_phis_rad_delta = torch.where(middle_cond, -(veh_vs / (CROSSROAD_SIZE/2-2.5*LANE_WIDTH)) / self.base_frequency, zeros)  # TODO：ONLY FOR 3LANE
        else:
...
def deal_with_phi_diff(phi_diff):
    phi_diff = torch.where(phi_diff > 180., phi_diff - 360., phi_diff)
    phi_diff = torch.where(phi_diff < -180., phi_diff + 360., phi_diff)
    return phi_diff
...
class ReferencePath(object):
    ...
    def future_n_data(self, current_indexs, n):
        ...
        for _ in range(n):
            current_indexs += 80
            current_indexs = torch.where(current_indexs >= len(self.path[0]) - 2, len(self.path[0]) - 2, current_indexs)
    ...
    def indexs2points(self, indexs):
        ...
        points = torch.gather(torch.Tensor(self.path[0]), 0,  indexs), \
    ...
    def tracking_error_vector(self, ego_xs, ego_ys, ego_phis, ego_vs, n):
        ...
        tracking_error = torch.stack([two2one(current_points[0], current_points[1]),
                                           deal_with_phi_diff(ego_phis - current_points[2]),
                                           ego_vs - self.exp_v], 1)

        final = tracking_error
        if n > 0:
            future_points = torch.cat([torch.stack([ref_point[0] - ego_xs,
                                                 ref_point[1] - ego_ys,
